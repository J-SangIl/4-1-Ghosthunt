  ...
  return (
    <div className="w-full bg-white/95 backdrop-blur-md rounded-2xl p-5 border-2 border-teal-200 shadow-md flex flex-col gap-4">
      ...
      {activeTutorialMode && tutorialStep === 0 ? (
        <div className="py-6 px-4 bg-teal-50/50 border-2 border-dashed border-teal-200 rounded-xl flex flex-col items-center justify-center gap-2.5 shadow-inner">
          <span className="text-xs sm:text-sm font-black text-teal-800 font-jua text-center">
            {activeTutorialMode === 'sniper' ? '🎯 저격 조준 훈련 (튜토리얼)' : '💥 다중 소탕 훈련 (튜토리얼)'}
          </span>
          <span className="text-[11px] text-slate-500 font-sans text-center">
            시작 버튼을 누르면 조준 및 소탕 실전 훈련이 활성화됩니다!
          </span>
          <button
            type="button"
            onClick={() => {
              if (activeTutorialMode === 'sniper') {
                ...
                onSetupSweeperStep?.(1);
              }
            }}
            className="px-5 py-2 font-jua text-[13px] bg-amber-500 hover:bg-amber-600 text-white rounded-lg cursor-pointer transition-all shadow-md hover:scale-[1.02] active:scale-[0.98] tutorial-focus-glow-amber animate-pulse"
          >
            훈련 개시하기 🚀
          </button>
        </div>
          ...
          {conditionGameState === 'revealed' ? (
            <div className={`relative w-full ${
              activeTutorialMode === 'sweeper' && tutorialStep && tutorialStep > 0
                ? (sweeperStepSuccess ? 'tutorial-focus-glow-emerald animate-pulse' : 'tutorial-focus-glow-rose') + ' rounded-xl'
                : ''
            }`}>
              <button
            ...
            <div className="flex flex-col gap-2">
              <div className={`relative w-full ${activeTutorialMode === 'sweeper' && tutorialStep && tutorialStep > 0 && conditionGameState === 'selecting' && selectedCount > 0 ? 'tutorial-focus-glow-rose rounded-xl' : ''}`}>
                <button
          ...
          <div className="flex flex-col items-center justify-center py-2 bg-teal-50/40 rounded-2xl border border-teal-100 p-2 relative">
            ...
            <div className="text-center mb-1 flex items-center gap-1 justify-center">
             ...
             <div className="flex items-center justify-center gap-1.5 w-full">
              ...
              {/* Real Text Input */}
              <div className={`relative ${activeTutorialMode === 'sniper' && tutorialStep && tutorialStep > 0 ? 'tutorial-focus-glow-teal rounded-xl' : ''}`}>
                <input
                  ref={inputRef}
                  type="text"
                  value={inputValue}
